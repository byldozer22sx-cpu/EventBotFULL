...
function normalizeSlots(value) {
  const fallback = getConfig().slots || 20;
  const number = Number(value ?? fallback);

  if (!Number.isFinite(number)) return 20;

  // Discord разрешает максимум 5 рядов по 5 кнопок = 25 кнопок в одном сообщении
...
function loadData() {
  try {
    if (!fs.existsSync(dataPath)) {
      return { taken: {}, panel: null, closed: false, slots: normalizeSlots() };
    }
...
async function sendPanel(interaction) {
  const selectedSlots = normalizeSlots(interaction.options.getInteger('slots'));
  const image = interaction.options.getAttachment('image');
  const data = loadData();

  if (image && image.contentType && !image.contentType.startsWith('image/')) {
    await interaction.reply({
      content: '❌ Нужно прикрепить именно картинку.',
      ephemeral: true
    });
    return;
  }
  ...
  // ВАЖНО: сначала сохраняем новое количество мест в data.json,
  // и только потом вызываем buildButtons(), потому что buildButtons() читает data.json.
  data.taken = {};
  data.closed = false;
  data.panel = null;
  ...
  if (image) {
    panelMessage.files = [image.url];
  }

  const message = await interaction.channel.send(panelMessage);
  ...
  await interaction.reply({
    content: image
      ? `✅ Панель записи создана. Мест: ${selectedSlots}. Фото прикреплено.`
      : `✅ Панель записи создана. Мест: ${selectedSlots}`,
    ephemeral: true
  });
}
